...
/** Outbound identity for a provider's video hop (catalog profile id + wire format). */
function videoTransport(provider) {
 ...
 * - Every hop goes through proxyAwareFetch with the registry identity wrap
 *   (xAI resolves to the grok-cli catalog profile).
...
export async function handleVideoProxyCore({
  ...
  const { identity, format } = videoTransport(provider);
  ...
  if (
    ...
    let refreshed = null;
    try {
      refreshed = await withCredentialRefreshLock(provider, credentials, () =>
