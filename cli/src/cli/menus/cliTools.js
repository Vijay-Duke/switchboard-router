...
async function getFirstApiKey() {
  const secret = await prompt("Paste client key secret (leave empty to cancel): ");
  return secret || null;
}
...
async function buildCodexHeader() {
  ...
  const { installed, hasSwitchboard, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasSwitchboard, settings } = result.data;
  ...
  // Extract Switchboard custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Switchboard-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Switchboard provider config
  const provider = settings?.models?.providers?.["switchboard"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("switchboard/") ? primary.replace("switchboard/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasSwitchboard, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Switchboard\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
