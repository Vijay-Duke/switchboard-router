/**
 * `switchboard xai video` — generate a Grok Imagine video through the local
 * Switchboard gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: switchboard xai video --prompt "..." [options]

Generate a Grok Imagine video via your local Switchboard gateway
  ...
  try {
    ...
    if (create.status !== 200 || !create.body?.request_id) {
      ...
      if (create.status === 400 && /No credentials/i.test(String(detail))) {
        console.error("   Connect an xAI account first: Switchboard dashboard → Providers → xAI (Grok).");
      }
      return 1;
    }

    const requestId = create.body.request_id;
    const connectionId = create.headers["x-switchboard-connection-id"] || null;
