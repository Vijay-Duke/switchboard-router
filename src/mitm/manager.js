...
// (prevents EBUSY on `npm i -g switchboard-router@latest` while MITM is running).
...
const ENCRYPT_SALT = "switchboard-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall switchboard.`);
    }
  }
