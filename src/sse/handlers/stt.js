...
export async function handleStt(request) {
  ...
  if (!CREDENTIALED_PROVIDERS.has(provider)) {
    const result = await handleSttCore({ clientKeyId, provider, model, formData, sttConfig: AI_PROVIDERS[provider]?.sttConfig });
  ...
  while (true) {
    ...
    const result = await handleSttCore({ clientKeyId, provider, model, formData, credentials, sttConfig: AI_PROVIDERS[provider]?.sttConfig });
