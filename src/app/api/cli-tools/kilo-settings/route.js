...
const hasSwitchboardConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["switchboard"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("switchboard");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasSwitchboard: hasSwitchboardConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Switchboard", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Switchboard settings removed from Kilo Code" });
  } catch (error) {
