...
const hasSwitchboardConfig = (config) => {
  ...
  return !!config.provider["switchboard"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["switchboard"];
...
export async function POST(request) {
  try {
    ...
    // Preserve any existing switchboard provider entry and its models
    const existingProvider = config.provider["switchboard"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["switchboard"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `switchboard/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `switchboard/${effectiveSubagentModel}`,
    };
...
// DELETE - Remove Switchboard provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["switchboard"]?.models) {
      delete config.provider["switchboard"].models[modelToRemove];
      ...
      if (Object.keys(config.provider["switchboard"].models).length === 0) {
        ...
        config.model = `switchboard/${remainingModels[0]}`;
      }
    } else {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Switchboard settings removed from OpenCode",
    });
  } catch (error) {
