 ...
 */
export default function ProvidersPageClient({ initialData }) {
  const [connections, setConnections] = useState(initialData?.connections || []);
  const [providerNodes, setProviderNodes] = useState(initialData?.nodes || []);
  ...
  // API Key section is always rendered (add custom), so never show a page-level empty state
  // unless the user is actively searching and nothing matches anywhere.
  const hasAnyResult =
    !searchQuery.trim() ||
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      ...
      {/* API Key Providers — built-in list + custom OpenAI/Anthropic compatible */}
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-xs text-text-muted mt-1">
              Built-in providers plus your own OpenAI- or Anthropic-compatible endpoints.
            </p>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative">
              <Button
                size="sm"
                icon="add"
                onClick={() => setShowAddCustomMenu((v) => !v)}
              ...
              {showAddCustomMenu && (
                <>
                  <button
                    ...
                    onClick={() => setShowAddCustomMenu(false)}
                  />
                  <div className="absolute right-0 top-full z-50 mt-1 w-64 rounded-xl border border-border bg-surface p-1 shadow-xl">
                    <button
                      type="button"
                      className="flex w-full items-start gap-2 rounded-lg px-3 py-2.5 text-left text-sm transition-colors hover:bg-bg"
                      onClick={() => {
                    ...
                    >
                      ...
                      <span>
                        <span className="block font-medium">OpenAI compatible</span>
                        <span className="block text-xs text-text-muted">
                          Any OpenAI-style base URL + API key
                        </span>
                      </span>
                    </button>
                    <button
                    ...
                    >
                      <span className="material-symbols-outlined text-[18px] text-primary mt-0.5">psychology</span>
                      <span>
                        <span className="block font-medium">Anthropic compatible</span>
                        <span className="block text-xs text-text-muted">
                          Messages API base URL + API key
                        </span>
                      </span>
                    </button>
                  </div>
        ...
        {/* Custom (user-defined) providers first */}
        ...
        {compatibleProviders.length === 0 &&
          anthropicCompatibleProviders.length === 0 &&
          apikeyEntries.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-3 py-8 border border-dashed border-border rounded-xl text-text-muted text-sm">
              <span className="material-symbols-outlined text-[28px]">extension</span>
              <p>No API key providers yet.</p>
