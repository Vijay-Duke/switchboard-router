...
/**
 * @typedef {{ ts: number, applied?: boolean, reason?: string, detail?: string,
 *   provider?: string, model?: string, tokensBeforeEst?: number,
 *   tokensAfterEst?: number, tokensSavedEst?: number, savedPct?: number,
 *   durationMs?: number }} PxpipeEvent
 * @typedef {{ requests: number, compressed: number, bypassed: number, errors: number,
 *   tokensBeforeEst: number, tokensAfterEst: number, tokensSavedEst: number,
 *   savedPct: number, imagesGenerated: number, compressionTimeMs: number,
 *   avgCompressionMs: number }} PxpipeWindowTotals
 * @typedef {{ installed: boolean, installing: boolean, version: string|null,
 *   running: boolean, uptimeMs: number, enabled: boolean, autoInstall: boolean,
 *   minChars: number, timeoutMs: number, mode: string }} PxpipeStatus
 */
...
export default function PxpipeClient() {
  /** @type {[PxpipeStatus|null, (s: PxpipeStatus|null) => void]} */
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Card className="p-4">
        ...
        {stats?.timeline?.some((/** @type {any} */ d) => d.tokensSavedEst > 0) ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={stats.timeline} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(/** @type {string} */ d) => d.slice(5)} />
              <YAxis tick={{ fontSize: 11 }} tickFormatter={fmtTokens} width={48} />
              <Tooltip formatter={(/** @type {number} */ v) => [fmtTokens(v), "Tokens saved"]} labelFormatter={(/** @type {string} */ d) => d} />
      ...
      <Card className="p-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(stats?.recent || []).slice(0, 50).map((/** @type {PxpipeEvent} */ ev, /** @type {number} */ i) => (
                <tr key={`${ev.ts}-${i}`} className="border-b border-border/50">
                  ...
                  <td className="py-1.5 pr-3 text-right font-mono text-xs">
                    {ev.applied ? fmtTokens(ev.tokensBeforeEst || 0) : "—"}
                  </td>
                  <td className="py-1.5 pr-3 text-right font-mono text-xs">
                    {ev.applied ? fmtTokens(ev.tokensAfterEst || 0) : "—"}
                  </td>
                  <td className="py-1.5 pr-3 text-right font-mono text-xs text-success">
                    {ev.applied ? fmtTokens(ev.tokensSavedEst || 0) : "—"}
                  </td>
                  <td className="py-1.5 pr-3 text-right font-mono text-xs">
