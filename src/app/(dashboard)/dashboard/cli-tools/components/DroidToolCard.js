...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any Switchboard model entry (support multi-model: custom:Switchboard-0, custom:Switchboard-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:Switchboard"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      if (existingModels.length > 0) {
        ...
        // Legacy: single model stored as custom:Switchboard-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:Switchboard-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:Switchboard"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasSwitchboard} loading={restoring}>
