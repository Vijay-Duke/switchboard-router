...
describe("resolveBaseUrl SSRF guard", () => {
  ...
  it("allows public http override and strips trailing slashes", () => {
  ...
  it("rejects private IP overrides", () => {
  ...
  it("rejects non-http protocols and unparseable URLs", () => {
    for (const bad of ["file:///etc/passwd", "gopher://127.0.0.1:70", "ftp://10.0.0.1", "not a url"]) {
      const params = { providerOptions: { baseUrl: bad } };
      expect(() => resolveBaseUrl(CONFIG, params), `should reject ${bad}`).toThrow();
    }
  });
});
