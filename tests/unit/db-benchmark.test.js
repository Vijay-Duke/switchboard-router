...
beforeAll(async () => {
  // SQLite setup
  tempSqlite = fs.mkdtempSync(path.join(os.tmpdir(), "switchboard-bench-sqlite-"));
  ...
  tempLowdb = fs.mkdtempSync(path.join(os.tmpdir(), "switchboard-bench-lowdb-"));
...
describeBenchmark("DB Benchmark — SQLite vs Lowdb", () => {
