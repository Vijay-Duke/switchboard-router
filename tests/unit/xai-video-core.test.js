 ...
 *
 * Covers:
 *  - registry wiring (videoConfig incl. outbound identity, grok-imagine-video kind)
 ...
 *  - abort/cancellation
 *  - outbound identity wrap via proxyAwareFetch
 */
...
vi.mock("../../open-sse/services/oauthCredentialManager.js", () => ({
  // Passthrough: lock contention is an integration concern, not tested here.
  withCredentialRefreshLock: (_provider, _credentials, fn) => fn(),
}));
...
describe("registry wiring", () => {
  it("exposes videoConfig for xai including the outbound identity", () => {
    expect(getVideoConfig("xai")).toEqual({
      identity: "grok-cli",
...
describe("handleVideoProxyCore", () => {
  ...
  it("forwards a creation POST byte-for-byte and passes request_id through", async () => {
    proxyAwareFetch.mockResolvedValueOnce(jsonResponse({ request_id: "req-123" }));
  ...
  it("forwards multipart bodies untouched with the original boundary header", async () => {
    proxyAwareFetch.mockResolvedValueOnce(jsonResponse({ request_id: "req-mp" }));
  ...
  it("url-encodes the request id when polling", async () => {
    proxyAwareFetch.mockResolvedValueOnce(jsonResponse({ status: "pending" }));
    await handleVideoProxyCore({
      provider: "xai",
      requestId: "id with/slash",
      credentials: { accessToken: "tok" },
    });
    expect(proxyAwareFetch.mock.calls[0][0]).toBe("https://api.x.ai/v1/videos/id%20with%2Fslash");
  });
  ...
  it("401 → refreshes once and retries once with the new token", async () => {
    ...
    expect(proxyAwareFetch.mock.calls[1][1].headers.Authorization).toBe("Bearer tok-NEW");
