...
SwitchboardをOpenAI Codex CLIと統合し、OpenAI APIリクエストをSwitchboardのインテリジェントルーティングシステム経由でルーティングします。
...
1. SwitchboardダッシュボードでAPIキーが正しいか確認
...
1. モデル名がSwitchboard設定と一致するか確認
2. SwitchboardダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにSwitchboardクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL=""
```

SwitchboardクラウドダッシュボードでAPIキーが設定されていることを確認してください。
