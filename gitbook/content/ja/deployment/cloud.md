...
リモートアクセスと本番利用のため、VPSまたはDockerにSwitchboardをデプロイ。
...
| `DATA_DIR` | `~/.switchboard` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/switchboard` を作成:
...
# リバースプロキシを使用しない場合、Switchboardポートを許可
