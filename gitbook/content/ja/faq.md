...
Switchboardに関する一般的な質問。

---

## Switchboardとは?

**Switchboardは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、Switchboard自体は100%無料でオープンソースです。**
...
**Switchboardはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Switchboardをセルフホストできますか?

**はい! Switchboardは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.switchboard`)
...
**はい、Switchboardはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.switchboard`(またはカスタム`DATA_DIR`)にローカル保存
- Switchboardサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
- **ドキュメント:** [switchboard.com/docs](/docs)
