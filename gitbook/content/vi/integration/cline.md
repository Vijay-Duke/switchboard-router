...
Tích hợp Switchboard với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Switchboard.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Switchboard đang chạy cục bộ hoặc cloud endpoint đã cấu hình
- API key từ Switchboard dashboard
...
Đặt base URL tới endpoint Switchboard:

**Cho Switchboard cục bộ:**
```
http://localhost:20128/v1
```

**Cho Switchboard cloud:**
```

```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint Switchboard
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key Switchboard của bạn
2. Bạn có thể tìm API key trong Switchboard dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-switchboard-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình Switchboard
...
Bạn có thể dùng bất kỳ model nào đã cấu hình trong Switchboard dashboard. Ví dụ phổ biến:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng Switchboard để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua Switchboard
...
3. Nhận giải thích AI qua Switchboard

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Switchboard để hiểu context và thực hiện thay đổi
...
1. Xác minh Switchboard đang chạy: `curl http://localhost:20128/health`
...
1. Xác minh API key trong Switchboard dashboard
2. Đảm bảo bạn sao chép đầy đủ key bao gồm prefix `sk-switchboard-`
3. Kiểm tra API key chưa hết hạn
4. Thử tạo API key mới

### Lỗi "Model Not Found"

1. Xác minh tên model khớp chính xác với cấu hình Switchboard
2. Kiểm tra kết nối provider đang hoạt động trong Switchboard dashboard
...
2. Xác minh Switchboard instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Switchboard để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng Switchboard cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: ``
2. Đảm bảo bạn đã cấu hình API key trong Switchboard cloud dashboard
...
2. **Theo dõi Usage**: Kiểm tra Switchboard dashboard để xem thống kê và chi phí
...
## Tích hợp với Tính năng Switchboard

### Định tuyến Model

Switchboard tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Switchboard tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Switchboard dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
