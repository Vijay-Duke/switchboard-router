...
Despliega Switchboard en VPS o Docker para acceso remoto y uso en producción.
...
export DATA_DIR="/var/lib/switchboard"
...
| `DATA_DIR` | `~/.switchboard` | Ruta de almacenamiento de la base de datos |
...
sudo chown $USER:$USER /var/lib/switchboard
```
...
# Iniciar Switchboard con PM2
pm2 start npm --name switchboard -- start
...
```bash
# Ver logs
pm2 logs switchboard

# Reiniciar aplicación
pm2 restart switchboard

# Detener aplicación
pm2 stop switchboard
...
Crea `/etc/nginx/sites-available/switchboard`:
...
# Si NO usas proxy reverso, permite los puertos de Switchboard
...
tar -czf switchboard-backup-$(date +%Y%m%d).tar.gz /var/lib/switchboard

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/switchboard-$(date +\%Y\%m\%d).tar.gz /var/lib/switchboard
```
...
# Ver logs
pm2 logs switchboard --lines 100
...
```bash
# Verificar si Switchboard está corriendo
pm2 status
