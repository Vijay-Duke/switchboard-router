...
Guía detallada de instalación de Switchboard con consejos de solución de problemas.
...
Instala Switchboard globalmente para usar desde cualquier lugar:
...
- ✅ Comando simple: `switchboard`
- ✅ Auto-actualizaciones con `npm update -g switchboard`
...
3. Se crea el directorio de datos en `~/.switchboard`
...
**Ubicación por defecto:** `~/.switchboard`
...
**Error:**
```
Error: EACCES: permission denied, mkdir '/usr/local/lib/node_modules/switchboard'
```
...
**Problema:** Switchboard usa demasiada RAM
...
```bash
npm install -g pm2
pm2 start switchboard --name switchboard
pm2 save
```
...
```bash
docker pull switchboard/switchboard:latest

docker run -d \
  -p 20128:20128 \
  -e JWT_SECRET="your-secure-secret" \
  -e INITIAL_PASSWORD="your-password" \
  -v switchboard-data:/root/.switchboard \
  --name switchboard \
  switchboard/switchboard:latest
```
...
nano ~/.bashrc  # o ~/.zshrc
# Eliminar exports relacionados con switchboard
```
