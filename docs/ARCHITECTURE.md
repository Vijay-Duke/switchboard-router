...
Switchboard is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Switchboard Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.switchboard/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.switchboard/usage.json`, `~/.switchboard/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.switchboard/db.json`)
- usage stats: `~/.switchboard/usage.json`
- request log lines: `~/.switchboard/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Switchboard Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.switchboard` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/switchboard && npm run build`
- Build Docker image: `cd /root/dev/switchboard && docker build -t switchboard .`
