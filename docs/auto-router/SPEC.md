# Auto Router — Full Specification
...
Auto Router adds a combo strategy **Auto** that:
...
**Capacity auto-switch** (heuristic pre-filter for vision/PDF) runs **before** the router when enabled — not a replacement for Auto.
...
| Worker fails | Optional: try alternates[0] once, then fallback chain |
...
Optional response headers (opt-in):
...
- Dashboard auth required for learn/insights APIs
...
2. Worker failure — one router alternate vs full fallback chain?
